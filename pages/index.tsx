import Head from "next/head";
   ...
   export default function Home() {
     ...
     // Lazy load desktop video only, with fallback
     useEffect(() => {
       const videos = document.querySelectorAll('video.vid-desktop');
       // Fallback: set src after 2 seconds if not loaded
       ...
       // If no IntersectionObserver, set src immediately
       videos.forEach((video: HTMLVideoElement) => {
         const source = video.querySelector('source') as HTMLSourceElement | null;
         if (source && source.dataset.src) {
           source.src = source.dataset.src;
     ...
     return (
       <div className="bg-black text-white min-h-screen relative font-sans">
         ...
         <section className="relative z-10 w-full hero-h flex flex-col justify-center items-center text-center overflow-hidden pt-16">
           <Image
             src="/sprinter.png" // Fallback image
             alt="Hero background"
             fill
           ...
           <video
             ...
             aria-hidden="true"
           >
             <source data-src="/v5.mp4" type="video/mp4" />
           </video>
